...
The result is the open source [Lisnoti](https://github.com/Lisnoti/Lisnoti/) (`/lɪzˈnəʊtiː/`) font, which can be **[downloaded here](https://github.com/Lisnoti/Lisnoti/#getting-lisnoti)**. It is also the font used to typeset this blog.
...
The need to distinguish similar-looking characters such as `I` v `l` is not just a coding issue -- see e.g. [this MyFonts article](https://www.myfonts.com/pages/linotype-neue-frutiger-1450) on redesigning Frutiger to comply with German legibility standard DIN 1450.
...
What this means is that, *even if `I` is distinct from `l` within a font, you cannot deduce which is which in isolation*.
